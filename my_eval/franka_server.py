...
class FrankaServer:
    ...
    def _set_currpos(self, msg):
        ...
        self.tau_J = np.array(list(msg.tau_J)).reshape((7,))
        self.tau_ext = np.array(list(msg.tau_ext_hat_filtered)).reshape((7,))
        try:
        ...
        return jsonify(
            {
                ...
                "tau_J": np.array(robot_server.tau_J).tolist(),
                "tau_ext": np.array(robot_server.tau_ext).tolist(),

...
class OpenPiSFTWrapper:
    def __init__(
        self, checkpoint_dir: str, config_name: str, action_dim: int = 7, default_prompt: str = "Do the task."
    ):
        """
        全功能 OpenPi SFT 模型适配器 (兼容 LeRobot 接口)
        """
        print("[OpenPiWrapper] Initializing...")
        ...
        self.policy = policy_config.create_trained_policy(self.cfg, checkpoint_dir, default_prompt=default_prompt)
    ...
    def _process_observation(self, observation: dict[str, Any], task: str | None = None) -> dict[str, Any]:
        """数据清洗与映射"""
        # 1. 确保全部是 Numpy
    ...
    def get_action_chunk(self, observation: dict[str, Any], task: str | None = None) -> np.ndarray:
        """
        单次推理，返回完整 Chunk。
        """
        # 1. 预处理

...
import pickle
import numpy as np
from collections import Counter, defaultdict  # <--- 新增 defaultdict
...
def main():
    ...
    if trend_counter:
        for trend, count in trend_counter.most_common():
            desc = ""
            if trend == "101": desc = "(完美: 开->合->开)"
            elif trend == "10":  desc = "(只抓未放: 开->合)"
            elif trend == "1":   desc = "(全程张开)"
            elif trend == "0":   desc = "(全程闭合)"
            elif trend == "01":  desc = "(开始闭合: 合->开)"
            elif trend == "NoTrigger": desc = "(数值未触发阈值)"

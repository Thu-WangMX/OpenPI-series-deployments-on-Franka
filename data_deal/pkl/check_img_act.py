    ...
    for ep_idx in range(min(NUM_EPISODES, dataset.num_episodes)):
        ...
        for i in range(from_idx, to_idx):
            ...
            if action is not None:
                ...
                # 在画面上叠加信息
                # 第一行：Z轴数值
                cv2.putText(img_bgr, f"Z: {z_val:.4f}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, color, 2)
                # 第二行：当前 Episode 和 帧号
                cv2.putText(
                    img_bgr, f"Ep: {ep_idx} | Frame: {i}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1
                )

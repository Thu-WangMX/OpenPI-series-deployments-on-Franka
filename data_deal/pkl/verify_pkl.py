        ...
        for i in range(num_frames):
            ...
            if i < num_frames - 1:
                ...
                if not np.isclose(current_gripper_action, expected_val, atol=1e-5):
                    errors.append(
                        f"帧 {i} (Action): 动作第7维 ({current_gripper_action}) 不等于下一帧 gripper_pose ({expected_val})"
                    )
            ...
            # 检查最后一帧：应等于倒数第二帧的 Action
                ...
                if not np.isclose(current_gripper_action, prev_action_val, atol=1e-5):
                    errors.append(
                        f"帧 {i} (Last Action): 动作第7维 ({current_gripper_action}) 不等于上一帧 Action ({prev_action_val})"
                    )
